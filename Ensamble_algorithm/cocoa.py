...
df = pd.read_excel("C://Users//user//Downloads//ensamble//Coca_Rating_Ensemble.xlsx")
...
# droping "Bean_Type " & "Origin" since having null value masking problm
df = df.iloc[:, 0:7]

#converting input and output columns into categorical
lb = LabelEncoder()
df["Name"]= lb.fit_transform(df["Name"])
df["Company_Location"] = lb.fit_transform(df["Company_Location"])
df["Company"] = lb.fit_transform(df["Company"])

## discretize ouput column 
df['Rating'].describe()  # max value = 5
bins=[0,1,2,3,4,5] 
group_names= ['low_[0-1]','medium_(1-2]','high_(2-3]','very high_(3-4]','extra_very_high_(4-5]']    # (a,b] => a not included; but b included 
df['Rating']= pd.cut(df['Rating'],bins, labels = group_names)

df.describe()

# applying normalisation on "company", "name", "REF","company location" & "Review" columns
def norm_func(i):
    x = (i-i.mean())/(i.std())
    return (x)
# Normalized data frame (considering the numerical part of data)
df_norm = norm_func(df[['Company', 'Name', 'REF', 'Review','Company_Location']])
df_norm.describe()
...
print(classification_report(y_test, hard_predictions_test))
print('Hard Voting:\n', confusion_matrix(y_test, hard_predictions_test))
print('Hard Voting:\n', accuracy_score(y_test, hard_predictions_test))
print(classification_report(y_train, hard_predictions_train))
...
print(classification_report(y_test, soft_predictions_test))
print('soft Voting:\n', confusion_matrix(y_test, soft_predictions_test))
print('soft Voting:\n', accuracy_score(y_test, soft_predictions_test))
print(classification_report(y_train, soft_predictions_train))
...
# converting ouput variable to numeric binary format
lb = LabelEncoder()
df1["Rating"] =lb.fit_transform(df1["Rating"])

# Input and Output Split
predictors = df1.loc[:, df1.columns!="Rating"]
type(predictors)
...
param_grid = {"max_features": [4, 5, 6, 7, 8, 9, 10], "min_samples_split": [2, 3, 4, 5,6,7,8,9,10],"max_leaf_nodes":[4,5,6]}
...
print(classification_report(y_test, cv_rf_clf_grid.predict(x_test)))
